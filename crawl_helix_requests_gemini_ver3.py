...
def crawl_helixcenter_roundtables():
    ...
    for year in range(12, 25):
        ...
        try:
            ...
            event_links = soup.select('.entry-content .su-accordion .su-spoiler-content ul li a')
            logging.debug(f"  Found {len(event_links)} event links on {year_url}")

            # Extra debugging: Print the HTML of each found link
            for link in event_links:
                logging.debug(f"    Link HTML: {link}")
...
if __name__ == "__main__":
    ...
    with open("helixcenter_gemini_ver4.json", "w") as f:
        json.dump(roundtable_data, f, indent=2)

    logging.info("Roundtable data saved to helixcenter_gemini_ver4.json")